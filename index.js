...
app.use((err, req, res, next) => {
  console.error('Ошибка:', err.message); // Логирование сообщения об ошибке
  console.error('Стек вызовов:', err.stack); // Логирование стека вызовов

  // Отправка ответа клиенту
  res.status(500).json({
    message: 'Произошла ошибка на сервере',
    error: process.env.NODE_ENV === 'production' ? null : err.message, // Не показывать детали ошибки в продакшене
  });
});
